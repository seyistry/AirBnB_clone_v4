    ...
    if (this.checked === false) {
      ...
      $.each(StateId, function (key, value) {
        if (value === l) {
          n++;
          $(
            '.locations > .popover > li > ul > li > input:checkbox[data-id^="' +
              key +
              '"]'
          ).prop('checked', false);
  ...
  $(document).on(
    ...
    function () {
      ...
      if (n === 0) {
        const l = $(this).attr('data-state');
        $(
          '.locations > .popover > li > input:checkbox[data-id^="' + l + '"]'
        ).prop('checked', true);
        $.getJSON(
          'http://172.23.179.134:5001/api/v1/states/' +
  ...
  $.ajax({
    ...
    success: (data) => {
      ...
      $('.reviewSpan').click(function (event) {
        $.ajax(
          'http://172.23.179.134:5001/api/v1/places/' +
            $(this).attr('data-id') +
            '/reviews'
        ).done(function (data) {
          ...
          if (
            $("span[data-id='" + FirstPlace.place_id + "']").hasClass(
              'hideReview'
            ) === false
          ) {
            for (const review of data) {
              $(".reviews ul[data-id='" + review.place_id + "']").append(
                `<li>${review.text}</li>`
              );
            }
            $("span[data-id='" + FirstPlace.place_id + "']").addClass(
              'hideReview'
            );
            ...
            $("span[data-id='" + FirstPlace.place_id + "']").removeClass(
              'hideReview'
            );
  ...
  $('button').click(function () {
    ...
    $.ajax({
      ...
      data: JSON.stringify({
        ...
        states: Object.keys(StatesCheked),
